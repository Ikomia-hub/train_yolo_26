...
from train_yolo_26.utils.ikutils import prepare_dataset
from train_yolo_26.utils import custom_callbacks

# Update a setting
settings.update({'mlflow': False})


class TrainYolo26Param(TaskParam):

    def __init__(self):
        TaskParam.__init__(self)
        dataset_folder = os.path.join(os.path.dirname(os.path.realpath(__file__)), "dataset")
        self.cfg["dataset_folder"] = dataset_folder
        self.cfg["model_name"] = "yolo26m"
        self.cfg["epochs"] = 100
...
class TrainYolo26ParamFactory(dataprocess.CTaskParamFactory):
    def __init__(self):
        dataprocess.CTaskParamFactory.__init__(self)
        self.name = "train_yolo_26"

    def create(self):
        return TrainYolo26Param()


class TrainYolo26(dnntrain.TrainProcess):

    def __init__(self, name, param):
        dnntrain.TrainProcess.__init__(self, name, param)
        if param is None:
            self.set_param_object(TrainYolo26Param())
        else:
    ...
    def run(self):
        ...
        # Conversion from Ikomia dataset to YOLO dataset
...
class TrainYolo26Factory(dataprocess.CTaskFactory):
    ...
    def __init__(self):
        ...
        self.info.repository = "https://github.com/Ikomia-hub/train_yolo_26"
    ...
    def create(self, param=None):
        ...
        return TrainYolo26(self.info.name, param)
